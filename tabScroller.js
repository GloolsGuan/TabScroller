    ...
    $.tabScroller = $.fn.tabScroller = function(options){
        ...
        */
        ...
        self.initContent = function(){
	        ...
	        self.r.oTabs = self.r.content.children().length;
        ...
        function setHandlerForTabs(){
            ...
            $(self.r.tabsMap).find('.tabsMap_item').each(function(){
                $(this).click(function(){
                    ...
                    self.goToNextTab(index, 'nature', 50);
                });
            });
        }
        ...
        function move(di, moveType, nPI){
            ...
            var d = new Date();
            var intervalTime = ('flow'!=moveType) ? 20 : 10 ;
            ...
            //alert(self.r.animations);
            eBeforeMove(nPI, moveType, intervalTime);
            self.goToNextTab(nPI, moveType, intervalTime);
            eAfterMove(nPI, moveType, intervalTime);
        ...
        */
        self.goToNextTab = function(nPI, moveType, intervalTime) {
            var moveHook = '';
            
            for(ts in self.r.animations) {
			    //alert('mT:'+moveType+'; list:'+self.r.animations[ts]+'/'+$.inArray(moveType, self.r.animations[ts]));
			    if ($.inArray(moveType, self.r.animations[ts])>=0) {
				    moveHook = 'tabExchange_'+ts;
				}
			}
			
			if (1>moveHook.length){
			    alert('Error: provided unknown move type + "'+ moveType + '".');
			}
			
			self[moveHook](nPI, moveType, intervalTime);
            //-----------------
            
        }
        
        self.tabExchange_path = function(nPI, moveType, intervalTime){
		    
		    self.r.content.children().css({
			    'float': 'left'
			});
		...
		self.tabExchange_film = function(nPI, moveType, intervalTime){
		    var func = 'fa_' + moveType;
		    self[func](nPI, moveType, intervalTime);
		}
        
        
        /*
          Film Animation(fa) - fadeIn
        */
        self.fa_fadeIn = function(nPI, moveType, intervalTime){
            var nTab = self.r.content.children().eq(nPI);
            var cTab = self.r.content.children().eq(self.r.cPI);
            /*
            var bgImg = $(cTab).find('.photo img');
            alert(nTab.attr('id') + ':' + bgImg.attr('src'));
            $(nTab).css({
            ...
            */
            //alert(cTab.attr('id') + ';' + nTab.attr('id'));
            self.r.content.children().fadeOut(1200);
            $(nTab).fadeIn(1200);
        ...
        */
        ...
        /*
            ...
            * Event: afterMove
        */
        function eAfterMove(nPI, moveType, intervalTime){
            
            //alert('cPI:'+self.r.cPI + "\nnPI:" + nPI +"\nTabs:" + self.r.tabs);
        ...
        */
        self._movePath_flow = function (nPI, moveType) {
            ...
            var c=r.tPs[r.cPI], n=r.tPs[nPI], mP =[], fraction=30, p=0, rM=0, lP=r.tPs[r.cPI],
            ...
            while (d>0) {
                if ('left'==di) {
                    lP = lP+fraction > n ? lP+(n-lP) : lP+fraction;
                }else{
                    lP = lP-fraction < n ? lP-(lP-n) : lP-fraction;
                }
